...
int main()
{
    ...
    std::vector<float> xHostData(15, 1);
    ret = CreateAclTensor(xHostData, xShape, &xDeviceAddr, aclDataType::ACL_FLOAT16, &x);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    std::vector<int64_t> targetShape = {3};
    std::vector<int32_t> targetHostData(3, 1);
    ...
    std::vector<float> weightHostData(5, 1);
    ...
    aclTensor* total_weight = nullptr;
    void* total_weightDeviceAddr = nullptr;
    ...
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    std::vector<int64_t> total_weightShape = {1};
    std::vector<float> total_weightHostData(1, 0);
    ret = CreateAclTensor(total_weightHostData, total_weightShape, &total_weightDeviceAddr, aclDataType::ACL_FLOAT16, &total_weight);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    

    // 调用 aclnnNllLossGetWorkspaceSize 第一段接口
    uint64_t workspaceSize = 0;
    aclOpExecutor* executor = nullptr;
    ret = aclnnNllLossGetWorkspaceSize(x, target, weight, "mean", -100, y, total_weight, &workspaceSize, &executor);
    CHECK_RET(ret == 0, LOG_PRINT("aclnnNllLossGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 调用 aclnnNllLoss 第二段接口
    ret = aclnnNllLoss(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == 0, LOG_PRINT("aclnnNllLoss failed. ERROR: %d\n", ret); return ret);
    ...
    aclDestroyTensor(total_weight);
    aclrtFree(total_weightDeviceAddr);
    if (workspaceSize > 0) {
